...
Для анализа и обучения доступны также закрытые часовые свечи:
`GET /api/market/history?pair=BTC/USDT&exchange=binance&timeframe=1h`.
Последние 30 дней сохраняются в `market_candles`. Endpoint
