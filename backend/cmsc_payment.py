...
class CMSCPaymentStore:
    ...
    def create(self, email: str, quote: dict[str, Any], intent_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            row = CMSCPaymentIntent(intent_id=intent_id, user_id=user.id, email=email, cmsc_amount=float(quote['cmsc_amount']), currency=str(quote['currency']).upper(), eur_per_payment_unit=float(quote['eur_per_payment_unit']), gross_payment=float(quote['gross_payment']), fee_rate=float(quote['fee_rate']), fee_amount=float(quote['fee_amount']), payable_amount=float(quote['payable_amount']), status='pending_payment')
            session.add(row)
    ...
    def _dict(row: CMSCPaymentIntent) -> dict[str, Any]:
        return {'intent_id': row.intent_id, 'email': row.email, 'cmsc_amount': row.cmsc_amount, 'currency': row.currency, 'gross_payment': row.gross_payment, 'fee_rate': row.fee_rate, 'fee_amount': row.fee_amount, 'payable_amount': row.payable_amount, 'status': row.status, 'provider': row.provider, 'provider_reference': row.provider_reference, 'confirmed_at': row.confirmed_at.isoformat() if row.confirmed_at else None, 'created_at': row.created_at.isoformat() if row.created_at else None}
