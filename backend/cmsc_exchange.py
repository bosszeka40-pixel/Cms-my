...
def quote_cmsc(amount_cmsc: float, currency: str, fee_rate: float = DEFAULT_FEE_RATE) -> dict:
    ...
    return {
        "cmsc_amount": round(float(amount_cmsc), 8),
        "cmsc_eur_rate": 1.0,
        "currency": currency,
        "eur_per_payment_unit": round(eur_per_payment_unit, 10),
        "gross_payment": round(gross_payment, 10),
        "fee_rate": round(fee_rate, 8),
        "fee_amount": round(fee, 10),
        "payable_amount": round(payable, 10),
...
def create_payment_intent(engine, email: str, amount_cmsc: float, currency: str, fee_rate: float) -> dict:
    ...
    context = (
        f"intent={intent_id}; cmsc={quote['cmsc_amount']:.8f}; currency={quote['currency']}; "
        f"gross={quote['gross_payment']:.10f}; fee={quote['fee_amount']:.10f}; "
        f"payable={quote['payable_amount']:.10f}; rate={quote['eur_per_payment_unit']:.10f}"
    )
