...
try:
    ...
    purchase_strategy_with_cmsc = None
...
def purchase_strategy(payload: StrategyPurchasePayload, request: Request):
    if purchase_strategy_with_cmsc is None:
        raise HTTPException(status_code=503, detail="Покупка стратегии временно недоступна.")
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="Не удалось проверить цену стратегии.") from exc
...
def cmsc_exchange_quote(payload: CmscExchangePayload, request: Request, _: None = Depends(cmsc_quote_rate_limit)):
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="Не удалось получить актуальный курс.") from exc
...
def cmsc_exchange_intent(payload: CmscExchangePayload, request: Request, _: None = Depends(cmsc_intent_rate_limit)):
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail="Не удалось создать платёжный intent.") from exc
